const loadMonoSDK = (): Promise<void> => {
  return new Promise((resolve, reject) => {
    ...
    // Check if the script is already being appended
    const existingScript = document.querySelector('script[src="https://sdk.mono.co/connect.js"]');
    if (existingScript) {
      existingScript.addEventListener('load', () => resolve());
      existingScript.addEventListener('error', () => reject(new Error('Failed to load Mono SDK')));
      return;
    }
